...
class SearchProduct(APIView):
  ...
  def get(self, request):
    ...
    if key not in [f.name for f in m.Products._meta.get_fields()]: return Response({"status": False, "error": f"Field '{key}' does not exist in Products model"})
    products = m.Products.objects.filter(**{f"{key}__icontains": value})
