...
class SearchCategory(APIView):
  ...
  def get(self, request):
    ...
    if not value: return Response({"status": False, "error": "'value' key must be provided!"})
    if key not in [f.name for f in m.Category._meta.get_fields()]: return Response({"status": False, "error": f"Field '{key}' does not exist in Products model"})
    Category = m.Category.objects.filter(**{f"{key}__icontains": value})
