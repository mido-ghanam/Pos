...
class ProfitStatsView(APIView):
    def get(self, request):
        ...
        profit_from_sales = SalesInvoiceItem.objects.filter(invoice__created_at__gte=start).aggregate(profit=Sum(ExpressionWrapper(F('subtotal') - (Coalesce(F('product__buy_price'), 0) * F('quantity')), output_field=DecimalField())))["profit"] or Decimal("0")

        # لحساب خسارة المرتجعات من الأرباح (الفارق بين البيع والشراء للسلع المرتجعة فقط)
        profit_lost_from_returns = ReturnInvoiceItem.objects.filter(invoice__created_at__gte=start, invoice__partner_type='sale').aggregate(profit=Sum(ExpressionWrapper(F('subtotal') - (Coalesce(F('product__buy_price'), 0) * F('quantity')), output_field=DecimalField())))["profit"] or Decimal("0")
...
class CashBoxView(APIView):
    ...
    def get(self, request):
        ...
        """
        cashbox = CashBox.objects.first()
        if not cashbox: return Response({"error": "CashBox not found"}, status=404)
        return Response({
