...
ApiErrorModel _handleError(DioException error) {
  switch (error.type) {
    ...
    case DioExceptionType.connectionError:
      return ApiErrorModel(message: "لا يوجد اتصال بالإنترنت", code: -6);
...
ApiErrorModel _parseServerError(DioException error) {
  try {
    ...
    if (data is Map<String, dynamic>) {
      // case 1 => {status:false, message:"xxx"}
      ...
      // case 2 => validation only {password:[...]}
      return ApiErrorModel(
        message: null,
        code: error.response?.statusCode,
        errors: data,
...
Map<String, dynamic> _extractValidationErrors(Map<String, dynamic> data) {
  // ✅ في بعض السيرفرات بيرجع {errors:{field:[...]}}
  if (data["errors"] is Map<String, dynamic>) {
    return Map<String, dynamic>.from(data["errors"]);
  }

  // ✅ أو يرجع مباشرة fields
  final copied = Map<String, dynamic>.from(data);
  copied.remove("message");
  copied.remove("status");
  copied.remove("code");
  return copied;
}
